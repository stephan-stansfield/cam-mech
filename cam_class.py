...
class CamGeneration:
    ...
    def __init__(self, gear_ratios, initial_gr, sit_angle=np.pi):
        ...
        self.input_angles = self.gear_ratios[:, 1] # take the second column of gear ratios to be the input angles
        self.ninterp = 360 # number of radii points to interpolate between
        # self.sit_angle = sit_angle
    ...
    def cam_pts(self, stroke=np.pi, convex=True):
        ...
        if convex:
            self.convex_cam_pts()
            self.plot_cams() # why isn't this called with stroke as an input?
    ...
    def plot_forces_percentage(self, file, torque=False, stroke=2*np.pi):
        ...
        # DEBUG
        # print("Shape of x_e: ", np.shape(x_e))
        # print("Shape of R: ", np.shape(R))
        # print("Shape of r: ", np.shape(r))
        # print("Shape of f_c: ", np.shape(f_c))
        # plt.figure()
        # plt.plot(x_e, f_e)
        # plt.xlabel('elastic band displacement (m)')
        # plt.ylabel('elastic band force (N)')
        # plt.title('band displacement vs force')
        ...
        # xc_knee_neg = np.linspace(xc_knee[-1], xc_knee[-1] + (self.ninterp - self.sit_ind) * ((xc_knee[-1] - xc_knee[0]) / self.sit_ind), self.ninterp - self.sit_ind) # works but slope is too steep
        ...
        # xc_knee = np.concatenate((xc_knee_neg, xc_knee))
        ...
        # DEBUG
        # print("Range of knee angles: ", np.ptp(knee_angles))
        # print("Flipped knee angle array: ", knee_angles_flip)
        # print("Normalized flipped knee percentage: ", (knee_angles_flip - np.min(knee_angles_flip)) / (np.ptp(knee_angles)) )
        # print("max displacement: ", x_c[-1])

        # print("angle_percentage: ", angle_percentage)
        # print("elastic_percentage: ", elastic_percentage)
        # print("f_c_new: ", f_c_new)

        
        # plt.figure()
        # plt.plot(percentage, angle_percentage)
        # plt.xlabel('Stance Percentage (%)')
        # plt.ylabel('Cam Angle (rad)')
    ...
    # def normalize_angles(self, file):
    #     angle = np.loadtxt(file, delimiter=',')
    #     print(angle)
